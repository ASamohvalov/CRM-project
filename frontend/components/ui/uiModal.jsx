  ...
  return (
    ...
    >
      <div
        ...
        data-id="modal"
      >
        <div className="bg-chocolate max-w-[600px] h-[600px] mx-auto rounded-2xl p-10 flex flex-col justify-center">
          {children}
        </div>
      </div>
    </div>
...
UIModal.AddWork = function ({ handler, children }) {
  ...
  return (
    <form onSubmit={(e) => handler(e, titleId, naming)} className="flex flex-col items-center">
      <h1 className="text-4xl text-milk mb-[20%]">{children}</h1>
      <UiInput
        type="text"
      ...
      <UiInput
        ...
        className="bg-white w-72"
      />
      <UiButton type="submit" className={"w-72 h-20 rounded-2xl text-milk text-2xl"}>Отправить</UiButton>
    </form>
  );
};
...
UIModal.GetWork = function ({ handler }) {
  ...
  return (
    <>
      <h1 className="text-4xl text-milk mb-[10%] text-center">Должности</h1>
    <div className="overflow-y-auto max-h-[100%] titles__list">
      {titles ? Object.values(titles)?.map((item) => (
        <div className="bg-white mb-5 flex flex-col items-center rounded-3xl mr-3" key={item.id}>
